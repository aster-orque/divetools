...
const FOOTER_HREFS = {
  company: ['/about', '/contact', '/medias', '/why-become-host', '/why-become-diver'],
  support: ['/faq', '/faq-pro', '/contact'],
  platform: ['/experiences', '/structures', '/organize-trip'],
}
...
export default function Footer({
  ...
  labels: {
    tagline: string
    sections: {
      company: { title: string; links: { label: string }[] }
      support: { title: string; links: { label: string }[] }
      platform: { title: string; links: { label: string }[] }
    }
    copyright: string
    poweredBy: string
    socials: { facebook: string; instagram: string; podcast: string }
  }
}) {
  const year = new Date().getFullYear()
  const ASTER_BASE = `https://www.asterdive.com/${locale === 'fr' ? 'fr' : 'en'}`

  const FOOTER_LINKS: Record<string, { label: string; href: string }[]> = {
    [labels.sections.company.title]: labels.sections.company.links.map((link, i) => ({
      label: link.label,
      href: `${ASTER_BASE}${FOOTER_HREFS.company[i]}`,
    })),
    [labels.sections.support.title]: labels.sections.support.links.map((link, i) => ({
      label: link.label,
      href: `${ASTER_BASE}${FOOTER_HREFS.support[i]}`,
    })),
    [labels.sections.platform.title]: [
      ...labels.sections.platform.links.slice(0, 3).map((link, i) => ({
        label: link.label,
        href: `${ASTER_BASE}${FOOTER_HREFS.platform[i]}`,
      })),
      {
        label: labels.sections.platform.links[3]?.label ?? 'Blog',
        href: 'https://blog.orque.co',
      },
    ],
  }
    ...
    <footer className="border-t border-gray-200 bg-gray-50 px-4 py-10 sm:px-6">
      <div className="mx-auto max-w-5xl">
          ...
          <div className="flex gap-3">
            {SOCIALS.map((s) => (
              <a
                ...
                aria-label={labels.socials[s.key]}
        ...
        <div className="mt-8 flex flex-col items-center justify-between gap-2 border-t border-gray-200 pt-6 text-xs text-gray-400 sm:flex-row">
          <p>&copy; {labels.copyright.replace('{year}', String(year))}</p>
          <p>
            {labels.poweredBy}{' '}
            <a
